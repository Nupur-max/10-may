...
import {ActivityIndicator,SafeAreaView, View, Text, StyleSheet, TouchableOpacity, Image, TextInput, Dimensions, Platform, Alert, Modal, Pressable, DrawerLayoutAndroidComponent } from 'react-native';
...
//console.log('dhfshgfghghdjg', name)
  ...
  const GetUserDetails = async() => {
    ...
    let temData = [];
    db.transaction(tx => {
      tx.executeSql('SELECT user_id,name,email,Contact,roster_id,roster_pwd,airline_type FROM userProfileData Where user_id = "'+user.id+'"', [], (tx, result) => {
        //setOffset(offset + 10);
   ...
   Alert.alert(resData.message);
    ...
    const selectingImage = () => {
     ...
     const InsertInUserProfileData = async() => {
      ...
      db.transaction(tx => {
        tx.executeSql(
          'INSERT INTO userProfileData (user_id, name, LicenceType, LicenceNumber, validity, Country, CountryCode, Contact)  VALUES ("'+user.id+'", "'+name+'", "'+lt+'", "'+ln+'", "'+date+'", "'+value+'", "'+code+'", "'+mn+'")',
