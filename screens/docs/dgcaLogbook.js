...
import { View, Text, TextInput, TouchableOpacity, Platform, SafeAreaView, StatusBar } from 'react-native';
    ...
    const getLogbookData = async () => {
        ...
        prePopulateddb.transaction(tx => {
            if (logOption == 'Logbook') {
                tx.executeSql('SELECT * from logbook WHERE user_id == "' + user.id + '" AND orderedDate BETWEEN "' + orderStart + '" AND "' + orderEnd + '" AND tag == "manual" AND aircraftReg != "SIMU" ORDER BY orderedDate ASC', [], (tx, result) => {
                ...
                tx.executeSql('SELECT * from logbook WHERE user_id == "' + user.id + '" AND orderedDate BETWEEN "' + orderStart + '" AND "' + orderEnd + '" AND tag == "manual" AND stl == "true" ORDER BY orderedDate ASC', [], (tx, result) => {
                    for (let i = 0; i <= result.rows.length; i++) {
                        temData.push({
                            id: result.rows.item(i).id,
                            tag: result.rows.item(i).tag,
                ...
                {period == 'preDefined' ?
                    <View style={{ width: '100%' }}>
                        <DropDownPicker
                            ...
                            dropDownContainerStyle={{ borderColor: "#266173",
