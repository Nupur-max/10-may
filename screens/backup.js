        ...
        .then(syncData => {
           ...
           syncData.data.map((data, index) => {
            setLoad(true)
              ...
              let temData = [];
              db.transaction(tx => {
              tx.executeSql('SELECT id,tag,date,aircraftType,from_lat,from_long,from_nameICAO,offTime,onTime,p1,p2,to_nameICAO,to_lat,to_long,outTime,inTime,orderedDate from logbook WHERE user_id = "'+user.id+'" ORDER BY orderedDate DESC limit 10', [], (tx, result) => {
                  //setOffset(offset + 10);
